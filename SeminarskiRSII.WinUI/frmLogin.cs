       ...
       ApiService _osobljeService = new ApiService("Osoblje");
        ...
        private async void btnSnimi_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var list = await _osobljeService.get<List<Model.Osoblje>>(new OsobljeSearchRequest() {korisnickoIme = ApiService.Username});
                Model.Osoblje item = list.Where(x => x.KorisnickoIme == ApiService.Username).FirstOrDefault();
                if (item != null)
                {
