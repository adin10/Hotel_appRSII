...
namespace SeminarskiRSII.WinUI.Gost
{
    public partial class frmGostDetalji : Form
    {
        ...
        {
            var result = await _service.get<List<Model.Grad>>(null);       // Uzimamo sve gradove
            result.Insert(0, new Model.Grad());                           // Ovo sluzi 
        ...
        private async void frmGostDetalji_Load(object sender, EventArgs e)
        {
            ...
            {
                var g = await _gost.getByID<Model.Gost>(_id);  // Uzimamo ID gosta i popunjavamo textBox sa postojecim podacima
                txtIme.Text = g.Ime;
