...
namespace SeminarskiRSII.WinUI.SobaOsoblje
{
    public partial class frmSobaOsobljeDetalji : Form
    {
        ...
        {
            var list = await _osoblje.get<List<Model.Osoblje>>(null);
            list.Insert(0, new Model.Osoblje());
        ...
        private async void btnSnimi_Click(object sender, EventArgs e)
        {
            ...
            if (_id.HasValue)
            {
                await _service.Update<Model.SobaOsoblje>(_id, insert);
                MessageBox.Show("Uspjesno ste uredili podatke");
            }
            else
            {
                await _service.Insert<Model.SobaOsoblje>(insert);
        ...
        private async void frmSobaOsobljeDetalji_Load(object sender, EventArgs e)
        {
            ...
            if (_id.HasValue)
            {
                var list = await _service.getByID<Model.SobaOsoblje>(_id);
